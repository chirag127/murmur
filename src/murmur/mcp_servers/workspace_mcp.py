...
mcp = FastMCP(
    "workspace_mcp", description="Built-in workspace server (files, git, tests)"
)
...
def search_code(query: str, path: str = ".") -> List[Dict[str, Any]]:
    ...
    try:
        proc = subprocess.run(
            ["grep", "-rn", query, path], text=True, capture_output=True
        )
...
def run_tests(test_path: str = ".") -> str:
    ...
    try:
        proc = subprocess.run(
            ["pytest", test_path, "-v"], text=True, capture_output=True
        )
...
def git_branch(branch_name: str) -> str:
    ...
    proc = subprocess.run(
        ["git", "checkout", "-b", branch_name], text=True, capture_output=True
    )
    if proc.returncode != 0:
        return subprocess.run(
            ["git", "checkout", branch_name], text=True, capture_output=True
        ).stderr
