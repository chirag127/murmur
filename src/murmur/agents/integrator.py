...
async def integrator_node(
    ...
    if state["step_count"] >= state.get("max_steps", 200):
        return {"session_status": "failed", "messages": [SystemMessage(content="Max steps exceeded in integrator.")]}

    logger.debug("Integrator spinning up. Merging task branches into main session branch.")
    ...
    if state.get("dry_run"):
        return {"session_status": "done", "messages": [SystemMessage(content="Dry run: skipped merge and commit.")]}
    ...
    return {"session_status": "done", "messages": [SystemMessage(content="Integration complete.")]}
