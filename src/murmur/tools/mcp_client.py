...
class MCPClient:
    ...
    async def __aenter__(self) -> "MCPClient":
        logger.debug(f"Starting MultiServerMCPClient with servers: {list(self._server_config.keys())}")
    ...
    def get_tools_for_role(self, role: str) -> List[BaseTool]:
        ...
        if role == "supervisor":
            ...
            allowed = ["workspace", "sequential-thinking", "context7", "ref", "docfork", "exa", "tavily", "linkup"]
        elif role == "refactor_worker":
            allowed = ["workspace", "sequential-thinking", "context7", "ref", "docfork", "exa", "tavily", "linkup", "kindly-web-search"]
        ...
        for t in self._tools_cache:
            ...
            if server_name in allowed:
                ...
                if role in ("supervisor", "planner", "review_agent"):
                    if t.name in ("workspace__write_file", "workspace__create_patch", "workspace__git_commit"):
                        continue
