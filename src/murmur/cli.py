...
app = Typer_App = typer.Typer(
    help="Murmur — Hierarchical Multi-Agent AI Coding CLI", no_args_is_help=True
)
console = Console()
...
async def _run_graph(task: str, config: AppConfig, _type: str = "refactor"):
    ...
    async with MCPClient(config) as mcp:
        ...
        console.print(
            f"[bold blue]🐦 Murmur Starting[/] | Session: {sid} | Task: {task}"
        )

        try:
            async for event in graph.astream(
                initial_state, thread, stream_mode="updates"
            ):
                for node, state_update in event.items():
                    ...
                    if "messages" in state_update and state_update["messages"]:
                        console.print(
                            f"[cyan]Agent msg:[/cyan] {state_update['messages'][-1].content}"
                        )
        except asyncio.CancelledError:
            console.print("[bold red]INTERRUPTED[/] | Run `murmur status` to resume.")
...
def run(
    ...
    dry_run: bool = typer.Option(
        False, "--dry-run", help="Evaluate diffs without applying them."
    ),
    no_commit: bool = typer.Option(
        False, "--no-commit", help="Skip committing final result."
    ),
...
def doc(
    module: Optional[str] = typer.Option(
        None, "--module", help="Path to limit doc gen."
    ),
    repo_path: str = typer.Option(".", help="Target repository directory."),
    verbose: bool = typer.Option(False, "-v", "--verbose", help="Emit debug logs."),
):
